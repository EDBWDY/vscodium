...
if [[ "${SHOULD_BUILD_REH}" != "no" ]]; then
  ...
  # The VSCodium source patch enables the documented Node global-navigator
  # compatibility argument before this output is minified. Verify that the
  # source-level change was applied rather than rewriting minified output.
  grep -Fq "args.push('--supportGlobalNavigator');" src/vs/server/node/extensionHostConnection.ts
